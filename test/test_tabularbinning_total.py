...
def test_tabularbinning_classification(embedding, backbone):
    ...
    from benchmark.pipelines import TabularBinningPipeLine
    
    args = get_args()
    args.embedding = embedding
    args.backbone = backbone

    pipeline = TabularBinningPipeLine(args, data, label, continuous_cols, category_cols, output_dim, metric, metric_hparams)
    pipeline.benchmark()

@pytest.mark.parametrize("embedding, backbone", [("identity", "mlp")])
def test_tabularbinning_regression(embedding, backbone):
...
if __name__ == "__main__":
    test_tabularbinning_classification("identity", "mlp")
    test_tabularbinning_regression("identity", "mlp")