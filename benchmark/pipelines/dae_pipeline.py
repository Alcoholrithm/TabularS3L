...
from ts3l.utils.dae_utils import DAEConfig, DAECollateFN, DAEDataset
from ts3l.utils import TS3LDataModule

from hparams_range.dae import hparams_range
...
class DAEPipeLine(PipeLine):
    ...
    def initialize(self):
        self.config_class = DAEConfig
        self.pl_module_class = DAELightning
    ...
    def fit_model(self, pl_module: TS3LLightining, config: Type[BaseConfig]):
        ...
        train_ds = DAEDataset(X = self.X_train, unlabeled_data = self.X_unlabeled, continuous_cols = self.continuous_cols, category_cols = self.category_cols)
        valid_ds = DAEDataset(X = self.X_valid, continuous_cols = self.continuous_cols, category_cols = self.category_cols)

        pl_datamodule = TS3LDataModule(train_ds, valid_ds, self.args.batch_size, train_sampler='random', train_collate_fn=DAECollateFN(config), valid_collate_fn=DAECollateFN(config))
        ...
        train_ds = DAEDataset(X = self.X_train, Y = self.y_train.values, continuous_cols=self.continuous_cols, category_cols=self.category_cols, is_regression=True if self.output_dim == 1 else False)
        valid_ds = DAEDataset(X = self.X_valid, Y = self.y_valid.values, continuous_cols=self.continuous_cols, category_cols=self.category_cols, is_regression=True if self.output_dim == 1 else False)
    ...
    def evaluate(self, pl_module: TS3LLightining, config: Type[BaseConfig], X: pd.DataFrame, y: pd.Series):
        ...
        test_ds = DAEDataset(X = X, continuous_cols=self.continuous_cols, category_cols=self.category_cols)
